...
import com.lihaodong.pdf.exception.FileNotFoundException;
import com.lihaodong.pdf.http.HttpListener;
import com.lihaodong.pdf.http.OkHttpClientManager;
import com.lihaodong.pdf.listener.OnDrawListener;
import com.lihaodong.pdf.listener.OnLoadCompleteListener;
import com.lihaodong.pdf.listener.OnPageChangeListener;
import com.lihaodong.pdf.model.PagePart;
import com.lihaodong.pdf.util.ArrayUtils;
import com.lihaodong.pdf.util.Constants;
import com.lihaodong.pdf.util.FileUtils;
import com.lihaodong.pdf.util.NumberUtils;
...
import static com.lihaodong.pdf.util.Constants.Cache.CACHE_SIZE;
...
public class PDFView extends SurfaceView {
    ...
    public PDFView(Context context, AttributeSet set) {
        super(context, set);
        filePath=context.getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS+"/pdf").getAbsolutePath();
    ...
    public void fromUrl(String fileUrl, final HttpListener httpListener) {
        try {
            OkHttpClientManager.downloadAsyn(fileUrl, null,filePath, new OkHttpClientManager.ResultCallback()
            {
                @Override
                ...
                public void onDownloading(int progress) {
                    httpListener.onLoading(progress);
                }
            });
        } catch (Exception e) {
            httpListener.onFailed(e);
    ...
    public void fromUrl(String fileUrl, String fileName, final HttpListener httpListener) {
        try {

            OkHttpClientManager.downloadAsyn(fileUrl,fileName, filePath, new OkHttpClientManager.ResultCallback()
            {
                @Override
                public void onError(Request request, Exception e) {
                    httpListener.onFailed(e);
    ...
    public void fromUrl(String fileUrl, String fileName, String filePath, final HttpListener httpListener) {
        try {
            OkHttpClientManager.downloadAsyn(fileUrl,fileName, filePath, new OkHttpClientManager.ResultCallback()
                    {
                        @Override
